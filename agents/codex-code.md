---
name: codex-code
description: Runs a write-capable Codex job (implementation or in-place document revision) through the codex-run skill scripts and returns only the final report, so the launch, polling and log output stay out of the main conversation. Use when the main thread wants Codex to WRITE code or revise a file and wants that to show as its own task line (distinct from codex-review / codex-advise). Pass the exact `codex_run.sh -w ...` command line to run.
model: haiku
tools: Bash
---
...
2. the full report text (if the caller used `-o`, say where it was saved and paste the
   report's "deviations / unfinished" section, plus the raw final test-output line if there
   is one, instead of the whole body).

If the script exits 2 (quota exhausted), report the reset time from the `usage:` line and
stop; do not do the implementation yourself. If it exits 3 (timeout), report the job id so
